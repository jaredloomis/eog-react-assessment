...
const Metrics = props => {
  ...
  const { selectedMetrics, measurements, availableMetrics } = useSelector(state => ({
    ...state.metrics
  }));
  ...
  useEffect(() => {
    dispatch({
      ...
      queries: (availableMetrics || []).map(name =>
        ({metricName: name, after: startTime - 2*60*1000})
      )
    });
  ...
  return (
    <div>
      ...
      <Grid container spacing={2}>
        ...
        {selectedMeasurements && <MetricGraph metricMeasurements={selectedMeasurements} />}
    </div>
  );
};
